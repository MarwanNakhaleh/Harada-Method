...
import { generateCellHelpPrompt } from "@/lib/prompt";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if this is a cell-specific help request
    if (body.type === "cell-help") {
      const { helpQuestion, cellVariant, existingContent, cellId } = body;
      
      // Generate a prompt specifically for helping with this cell
      const cellPrompt = generateCellHelpPrompt(
        helpQuestion,
        cellVariant,
        existingContent,
        cellId
      ...
      return Response.json({
        recommendation: result.text.trim(),
        usage: result.providerMetadata?.openrouter?.usage,
      });
    }

    // Original questionnaire-based recommendations logic
    const { helpQuestion, cellVariant, existingContent, cellId } = body;

    const prompt = generateCellHelpPrompt(helpQuestion, cellVariant, existingContent, cellId);
